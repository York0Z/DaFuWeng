...
CentralWidget::CentralWidget(int _pn, int* _pp, QWidget *parent, Player* playersm[4], GUIPlayer* guiPlayers[4]): QWidget(parent)
{   //_pp[3]
    //pass in parameters to constructor
    currentPlayer = 0;
    //sets currenPlayer to 0
    players = playersm;
    pn = _pn;
    pp = _pp;
    gplayers = guiPlayers;
    ...
    boardLabel->setPixmap( QPixmap(":/images/board.jpg") );
    //board picture
    ...
    fill(upgradeOrNot, upgradeOrNot+40, false);
...
void CentralWidget::drawCard()
{
    ...
    //moving the players after the card
}
...
void CentralWidget::payProperty()
{
    ...
    buyProperty->setIcon( *property );
    buying = true;
    players[currentPlayer]->payMoney(amount);
    ...
    players[currentPlayer]->addPro();
    gplayers[currentPlayer]->updateLabel(players[currentPlayer]->getSpace(),
                                         players[currentPlayer]->printmoney(),players[currentPlayer]->getPro());
    ...
    if (players[currentPlayer]->printmoney() < 0)
    {
        ...
        gameEnds.exec();
        QApplication::quit();
        //quit the game if player has less than $0
    }
}
...
void CentralWidget::payupgradeProperty()
{
    ...
    if (upgradeOrNot[players[currentPlayer]->getSpace()] == false)
    {
        upgradeProperty->setIcon( *niceproperty );
        upgrading = true;
        board[players[currentPlayer]->getSpace()]->upgrade = true;
        //sets the upgrade to true with the bool in GUISpace.h

        QMessageBox payupgrade;
        //message box
        payupgrade.setText(QString("You just upgraded your space!     "));
        payupgrade.setGeometry(435,315,100,100);
        payupgrade.exec();

        upgradeOrNot[players[currentPlayer]->getSpace()] = true;
    }
...
void CentralWidget::endplayerTurn()
{
    ...
    {
        currentPlayer++;
        //increments the currentplayer
    }
    else {
        currentPlayer = 0;
        //sets currentplayer back to 0
    }
...
void CentralWidget::rollDice()
{
    ...
    players[currentPlayer]->updateSpace(num);
    ...
    if (newSpace > 40)
    {
        //if it passes the origin space 40
        int different = newSpace-40;
        //subtract it by 40
        players[currentPlayer]->updateSpace(-newSpace);
        //-n);
        players[currentPlayer]->updateSpace(different);
        newSpace = players[currentPlayer]->getSpace();
        //gets the space for newSpace
    }
    ...
    std::cout << "Y: " << y << std::endl;
    movePlayers(currentPlayer,x,y);
    amount = board[newSpace-1]->getamount();
    //gets the amount $ for the space
    ...
    if (newSpace == 34)
    {
        ...
        drawCard();
        //space for community chest
    }

    if (newSpace ==2 || newSpace ==4 || newSpace ==5
            || newSpace ==7 || newSpace ==9
            || newSpace ==10 || newSpace ==12
            || newSpace ==14 || newSpace ==15
            || newSpace ==16 || newSpace ==17
            || newSpace ==20 || newSpace ==22
            || newSpace ==24 || newSpace ==25
            || newSpace ==27 || newSpace ==28
            || newSpace ==30 || newSpace ==32
            || newSpace ==33 || newSpace ==34
            || newSpace ==36 || newSpace ==38
            || newSpace ==39 || newSpace ==40)
    {
        buyProperty->setEnabled(true);
        //these are spaces that are properties
        cardDeckButton->setEnabled(false);
        cardDeckButton2->setEnabled(false);
    }

    if (newSpace ==1 || newSpace ==3 || newSpace ==6
            || newSpace ==8 || newSpace ==11
            || newSpace ==13 || newSpace ==18
            || newSpace ==19 || newSpace ==21
            || newSpace ==23 || newSpace ==26
            || newSpace ==29 || newSpace ==31
            || newSpace ==34 || newSpace ==35
            || newSpace ==37)
    {
        buyProperty->setEnabled(false);
        //these are spaces that are not properties
    }

    if (newSpace ==1)
    {
        //collect 200 on go adds money to player
        players[currentPlayer]->getMoney(200);
        QMessageBox landongo;
        //message box
    ...
    if (newSpace ==31)
    {
        //player goes to jail if they land on space 31
        players[currentPlayer]->setSpace(11);
        movePlayers(currentPlayer,x,y);
        QMessageBox landonjail;
        //message box
    ...
    if (board[players[currentPlayer]->getSpace()-1]->checkOwner() == players[currentPlayer])
    {
        ...
        upgradeProperty->setEnabled(true);
        //can upgrade
        QMessageBox sameOwner;
        //message box
    ...
    else if (owner !=NULL)
    {
        ...
        {
            players[currentPlayer]->payMoney(0.50*(amount));
            //pays 50% of space price if upgraded by another player
            buyProperty->setEnabled(false);
            QMessageBox payRent2;
            //message box
            ...
            players[currentPlayer]->payMoney(0.20*(amount));
            //pay normal 20% rent
    ...
    QMessageBox gameTurn;
    //message box for ending turn
    ...
    gplayers[currentPlayer]->updateLabel(players[currentPlayer]->getSpace(),
                                         players[currentPlayer]->printmoney());
...
{
    diceButton->setEnabled(true);
    //enables dice for rolling at start
}

void CentralWidget::runTurn()
{
    //turn progression for player
    rollDice();
    movePlayers(currentPlayer, x, y);
    diceButton->setEnabled(false);
}
